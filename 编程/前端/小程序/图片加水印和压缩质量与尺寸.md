...
业务需要拍摄图片后，添加水印以及压缩大小后上传。

步骤：

1. 通过 [wx.chooseImage](https://developers.weixin.qq.com/miniprogram/dev/api/wx.chooseImage.html?search-key=chooseimage) 接口选择相机图片
2. 通过 [wx.getImageInfo](https://developers.weixin.qq.com/miniprogram/dev/api/wx.getImageInfo.html?search-key=getimage) 接口获取图片信息（长宽，类型）
3. 计算压缩比例和最终图片的长宽
4. 创建 [canvas](https://developers.weixin.qq.com/miniprogram/dev/component/canvas.html) ，绘制图片和添加水印
5. 通过 [wx.canvasToTempFilePath](https://developers.weixin.qq.com/miniprogram/dev/api/wx.canvasToTempFilePath.html) 接口将画布内容导出为图片并获取图片路径

由于有多个页面都需要同样的功能，所以将其封装为组件。
